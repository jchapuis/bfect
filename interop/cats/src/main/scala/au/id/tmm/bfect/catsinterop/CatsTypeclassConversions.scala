...
private[catsinterop] object TmmToCatsTypeclassConversionsImpls {
  ...
  class CatsBracketForTmmBracket[F[+ _, + _]](implicit tmmBracket: Bracket[F])
      extends CatsMonadErrorForTmmBME[F, Throwable]
      with cats.effect.Bracket[F[Throwable, +?], Throwable] {
    override def bracketCase[A, B](
        acquire: F[Throwable, A],
    )(use: A => F[Throwable, B])(release: (A, cats.effect.ExitCase[Throwable]) => F[Throwable, Unit]): F[Throwable, B] = {

      val releaseForTmmBracket: (A, ExitCase[Throwable, B]) => F[Nothing, _] = {
  ...
  class CatsSyncForTmmSync[F[+ _, + _]](implicit tmmSync: Sync[F])
      extends CatsBracketForTmmBracket[F]
      with cats.effect.Sync[F[Throwable, +?]] {
    override def suspend[A](thunk: => F[Throwable, A]): F[Throwable, A] = tmmSync.suspend(thunk)
  }

  class CatsAsyncForTmmAsync[F[+ _, + _]](implicit tmmAsync: Async[F])
      extends CatsSyncForTmmSync[F]
    ...
    override def asyncF[A](k: (Either[Throwable, A] => Unit) => F[Throwable, Unit]): F[Throwable, A] =
      tmmAsync.asyncF[Throwable, A](k.andThen(makeFailureUnchecked(_)))
  }

  class CatsConcurrentForTmmConcurrent[F[+ _, + _]](implicit tmmConcurrent: Concurrent[F])
      extends CatsAsyncForTmmAsync
    ...
    override def start[A](fa: F[Throwable, A]): F[Throwable, cats.effect.Fiber[F[Throwable, +?], A]] =
      tmmConcurrent.map(tmmConcurrent.start(fa))(asCatsFiber)
    ...
    override def cancelable[A](k: (Either[Throwable, A] => Unit) => cats.effect.CancelToken[F[Throwable, +?]]): F[Throwable, A] =
      tmmConcurrent.cancelable(k.andThen(makeFailureUnchecked(_)))
  }

  object CatsConcurrentForTmmConcurrent {

    def asCatsFiber[F[+ _, + _], A](tmmFibre: Fibre[F, Throwable, A]): cats.effect.Fiber[F[Throwable, +?], A] =
...
trait TmmToCatsTypeclassConversions {
  ...
  implicit def tmmBifunctorMonadIsCatsMonad[F[+ _, + _]: BifunctorMonad, E]: cats.Monad[F[E, +?]] =
    new CatsMonadForTmmBifunctorMonad[F, E]()
  implicit def tmmBifunctorMonadErrorIsCatsMonadError[F[+ _, + _]: BifunctorMonadError, E]: cats.MonadError[F[E, +?], E] =
    new CatsMonadErrorForTmmBME[F, E]()
  implicit def tmmBracketIsCatsBracket[F[+ _, + _]: Bracket]: cats.effect.Bracket[F[Throwable, +?], Throwable] =
    new CatsBracketForTmmBracket[F]()
  ...
  implicit def tmmConcurrentIsCatsConcurrent[F[+ _, + _]: Concurrent]: cats.effect.Concurrent[F[Throwable, +?]] =
    new CatsConcurrentForTmmConcurrent[F]()
